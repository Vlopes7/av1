  ...
  while (true) {
    ...
    switch (opcao) {
      case "1":
        ...
        const tipoKeys = Object.keys(tipoAeronave).filter((key) =>
          isNaN(Number(key))
        );
        const tipoIndex = readlineSync.keyInSelect(
          tipoKeys,
          "Tipo de aeronave:"
        );
  ...
  while (true) {
    ...
    switch (opcao) {
      case "1":
        ...
        const tipoPecaKeys = Object.keys(tipoPeca).filter((key) =>
          isNaN(Number(key))
        );
        const tipoPecaIndex = readlineSync.keyInSelect(
          tipoPecaKeys,
          "Tipo da peca:"
        );
        const tipo = tipoPeca[tipoPecaKeys[tipoPecaIndex]];
        const fornecedor = readlineSync.question("Fornecedor: ");
        const idAeronave = readlineSync.questionInt(
          "ID da aeronave associada: "
        );
        const novaPeca = new Peças(
          nomePeca,
          tipo,
          fornecedor,
          statusPeca.PRODUÇÃO,
      ...
      case "2":
        ...
        break;
      case "3":
        const nomeAtualizar = readlineSync.question(
          "Digite o nome da peca para atualizar: "
        );
        const statusKeys = Object.keys(statusPeca).filter((key) =>
          isNaN(Number(key))
        );
        const statusIndex = readlineSync.keyInSelect(
          statusKeys,
          "Novo Status:"
        );
  ...
  while (true) {
    ...
    switch (opcao) {
      case "1":
        ...
        const hierarquiaKeys = Object.keys(hierarquia).filter((key) =>
          isNaN(Number(key))
        );
        const hierarquiaIndex = readlineSync.keyInSelect(
          hierarquiaKeys,
          "Hierarquia:"
        );
  ...
  while (true) {
    ...
    switch (opcao) {
      case "1":
        ...
        const idAeronave = readlineSync.questionInt(
          "ID da Aeronave para a etapa: "
        );
        const nomesFuncionarios = readlineSync
        ...
        sistema.criarEtapa(
          ...
          new Date(dataStr),
          producao.PENDENTE,
          nomesFuncionarios.map((n) => n.trim()),
          idAeronave
        );
      ...
      case "2":
        ...
        break;
      case "3":
        const nomeIniciar = readlineSync.question(
          "Nome da etapa para INICIAR: "
        );
        const etapaIniciar = sistema.etapas.find(
          (e) => e.nome.toLowerCase() === nomeIniciar.toLowerCase()
        );
        if (etapaIniciar) {
        ...
        break;
      case "4":
        const nomeFinalizar = readlineSync.question(
          "Nome da etapa para FINALIZAR: "
        );
        const etapaFinalizar = sistema.etapas.find(
          (e) => e.nome.toLowerCase() === nomeFinalizar.toLowerCase()
        );
        if (etapaFinalizar) {
...
function realizarTeste() {
  ...
  const resultIndex = readlineSync.keyInSelect(
    resultKeys,
    "Resultado do teste:"
  );
...
function gerarRelatorio() {
  ...
  const idAeronave = readlineSync.questionInt(
    "ID da Aeronave para gerar relatorio: "
  );
